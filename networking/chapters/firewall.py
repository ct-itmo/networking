...
async def commit_correct_attempt(
    session: AsyncSession, meta: DockerMeta, task: str
) -> None:
    attempt = Attempt(
...
class FirewallVariant:
    ...
    def __init__(self, user_id: int):
        ...
        self.ip4_a_firewall = IPAddress(
            self.ip4_a_network.first | 1, self.ip4_a_network.version
        )
        self.ip4_a_client, self.ip4_a_client2, self.ip4_a_free, ip4_a_checker = (
            util.generate_distinct(
                4, util.generate_address, rnd, self.ip4_a_network, no_gateway=True
            )
        )

        self.ip4_b_firewall = IPAddress(
            self.ip4_b_network.first | 1, self.ip4_a_network.version
        )
        self.ip4_b_client, self.ip4_b_free, ip4_b_checker, self.ip4_b_client2 = (
            util.generate_distinct(
                4, util.generate_address, rnd, self.ip4_b_network, no_gateway=True
            )
        )

        udp_ports_number = util.generate_distinct(10, rnd.randint, 2001, 3000)
        self.allow_udp_port2 = udp_ports_number[0]
        deny_udp_addresses = addresses_list(
            *map(lambda p: (self.ip4_b_client2, p), udp_ports_number[1:])
        )
        udp_ports_client_listen = addresses_list(
            *map(lambda p: (self.ip4_b_client2, p), udp_ports_number)
        )
        ...
        def make_checker_meta(
            name: str,
            network_type: Literal["A", "B"],
            check_mode: str = "basic",
            environment: dict[str, str] = {},
            task: str | None = None,
            add_gateway: bool = True,
            add_socket_volume: bool = False,
        ) -> ContainerMeta:
            env = {
            ...
            if add_gateway:
                env["BOX_GATEWAY"] = str(
                    self.ip4_a_firewall if network_type == "A" else self.ip4_b_firewall
                )
            ...
            containers=[
                ...
                ContainerMeta(
                    ...
                    environment={
                        ...
                        "TCP_SERVERS": addresses_list(
                            (self.ip4_a_client, 3002), (self.ip4_a_client2, 3333)
                        ),
                    },
        ...
        self.checks = {
                ...
                [
                    make_checker_meta(
                        ...
                        environment={
                            ...
                            "PING_VALID_IPS": str(self.ip4_b_firewall)
                            + ","
                            + str(self.ip4_b_client),
                            "PING_INVALID_IPS": str(self.ip4_a_firewall)
                            + ","
                            + str(self.ip4_a_client),
                        },
                        add_gateway=False,
                ...
                [
                    make_checker_meta(
                        ...
                        environment={
                            ...
                            + ","
                            + str(self.ip4_b_client),
                            "UDP_VALID_ADDRESSES": addresses_list(
                                (self.ip4_b_client, 3001)
                            ),
                            "TCP_VALID_ADDRESSES": addresses_list(
                                (self.ip4_b_client, 3002)
                            ),
                        },
                        add_socket_volume=True,
                    )
            ...
            "forward_b_to_a": Check(
                [
                    make_checker_meta(
                        ...
                        task="forward_b_to_a",
                        environment={
                            "PING_VALID_IPS": str(self.ip4_a_firewall)
                            + ","
                            + str(self.ip4_a_client),
                            "UDP_VALID_ADDRESSES": addresses_list(
                                (self.ip4_a_client, 3001)
                            ),
                        },
                        add_socket_volume=True,
                    )
            ...
            "tcp_unidirectional": Check(
                [
                    make_checker_meta(
                        ...
                        environment={
                            ...
                            + str(self.ip4_b_client),
                            "UDP_VALID_ADDRESSES": addresses_list(
                                (self.ip4_a_client, 3001), (self.ip4_b_client, 3001)
                            ),
                            "TCP_VALID_ADDRESSES": addresses_list(
                                (self.ip4_a_client, 3002), (self.ip4_b_client, 3002)
                            ),
                        },
                        add_socket_volume=True,
                    ),
                    make_checker_meta(
                        ...
                        environment={
                            ...
                            "TCP_INVALID_ADDRESSES": addresses_list(
                                (self.ip4_a_client, 3002), (self.ip4_a_client2, 3333)
                            ),
                        },
                        add_socket_volume=True,
                ...
                {0: "/out/check.log", 1: "/out/check.log"},
                self.check_tcp_unidirectional,
            ),
            "udp_ports": Check(
                [
                    make_checker_meta(
                    ...
                    ContainerMeta(
                        ...
                        environment={
                            ...
                            "BOX_GATEWAY": str(self.ip4_b_firewall),
                            "TIMEOUT": "10s",
                            "UDP_SERVERS": addresses_list((self.ip4_b_client2, 3001))
                            + ","
                            + udp_ports_client_listen,
                            "HIDE_REQUEST_SOURCE": "true",
                        },
                        volumes=util.socket_volume(),
                    ),
                ],
                {0: "/out/check.log", 1: "/out/check.log", 2: "/out/check.log"},
                self.check_udp_ports,
                logs_joiner=lambda container_logs: (
                    f"=== from network B:\n{container_logs[1]}\n"
                    f"=== from network A:\n{container_logs[0]}\n"
...
class FirewallChapter(CheckableMixin, DockerMixin, BaseChapter[FirewallVariant]):
    ...
    deadline = datetime(2026, 5, 22, 21, 0, 0)
    tasks = [
