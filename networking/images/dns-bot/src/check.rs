...
use hickory_client::proto::rr::rdata::{A, AAAA, MX, NS};
...
#[async_trait]
trait Test {
    async fn check(&self, client: &mut Client) -> Result<(), Error>;
}
...
impl Test for FullMatchTest {
    async fn check(&self, client: &mut Client) -> Result<(), Error> {
        ...
        if response.answer_count() as u64 != self.expected_answer.len() as u64 {
            return Err(From::from(ErrorKind::WrongResponse(
                "Wrong number of records".to_string(),
            )));
        }

        let actual_answer: BTreeSet<_> = response
            .answers()
            .iter()
            .map(|r| r.data())
            .cloned()
...
impl Test for SoaTest {
    async fn check(&self, client: &mut Client) -> Result<(), Error> {
        ...
        match response.answers()[0].data() {
            RData::SOA(data) => {
                if data.mname() != &self.expected_mname || data.rname() != &self.expected_rname {
                    return Err(From::from(ErrorKind::WrongResponse(
                        "Wrong record content".to_string(),
                    )));
                }
            }
            _ => {
                return Err(From::from(ErrorKind::WrongResponse(
                    "Wrong record type".to_string(),
                )));
            }
        }
...
impl<'u> UdpChecker<'u> {
    ...
    pub async fn check_recursive(&mut self) -> Result<(), Error> {
        ...
        let test2 = FullMatchTest::new(
            ...
            BTreeSet::from([
                ...
                RData::NS(NS(Name::from_str("ns5.itmo.ru.")?)),
            ]),
        );
        ...
        let test3 = FullMatchTest::new(
            ...
            BTreeSet::from([RData::AAAA(AAAA(Ipv6Addr::from_str(
        ...
        let test4 = FullMatchTest::new(
            ...
            BTreeSet::from([RData::A(A(Ipv4Addr::new(85, 142, 29, 26)))]),
        );
        ...
        let test6 = FullMatchTest::new(Query::new("abacaba.ba.", RecordType::A)?, BTreeSet::new());
    ...
    pub async fn check_authoritative(&mut self) -> Result<(), Error> {
        ...
        let test1 = FullMatchTest::new(
            ...
            BTreeSet::from([RData::A(A(Ipv4Addr::from_str(ip4.as_str())?))]),
        );
        test1.check(&mut self.udp_client).await?;

        let test2 = FullMatchTest::new(
            Query::new(domain.as_str(), RecordType::AAAA)?,
            BTreeSet::from([RData::AAAA(AAAA(Ipv6Addr::from_str(ip6.as_str())?))]),
        );
        test2.check(&mut self.udp_client).await?;
    ...
    pub async fn check_subdomain(&mut self) -> Result<(), Error> {
        ...
        let test1 = FullMatchTest::new(
            ...
            BTreeSet::from([RData::A(A(Ipv4Addr::new(10, 52, 1, 1)))]),
        );
        test1.check(&mut self.udp_client).await?;

        let test2 = FullMatchTest::new(
            Query::new(domain.as_str(), RecordType::NS)?,
            BTreeSet::from([RData::NS(NS(Name::from_str(ns_domain.as_str())?))]),
        );
        test2.check(&mut self.udp_client).await?;
...
impl<'t> TcpChecker<'t> {
    pub async fn new(
        ...
        let (tcp_client, bg) = match Client::new(stream, sender, None).await {
            Ok(client) => client,
            Err(err) => {
                info!("Cannot connect to DNS: {}", err);
                return Err(From::from(err));
    ...
    pub async fn check_transfer(&mut self) -> Result<(), Error> {
        ...
        if zone_length < 2 {
            return Err(From::from(ErrorKind::WrongResponse(
                "Invalid zone transfer".to_string(),
            )));
        }
        ...
        match first.data() {
            RData::SOA(data) => {
                ...
                {
                    return Err(From::from(ErrorKind::WrongResponse(
                        "Wrong SOA record content".to_string(),
                    )));
                }
            }
            _ => {
                return Err(From::from(ErrorKind::WrongResponse(
                    "Wrong SOA record type".to_string(),
                )));
            }
        }

        let records: BTreeSet<_> = response.answers()[1..zone_length - 1]
            .iter()
        ...
        let expected = BTreeSet::from([
            (
                ...
                RData::NS(NS(Name::from_str(ns_domain.as_str())?)),
            ),
            (
                Name::from_str(domain.as_str())?,
                RData::A(A(Ipv4Addr::from_str(ip4.as_str())?)),
            ),
            (
                Name::from_str(domain.as_str())?,
                RData::AAAA(AAAA(Ipv6Addr::from_str(ip6.as_str())?)),
            ),
            (
                Name::from_str(domain.as_str())?,
                RData::MX(MX::new(10, Name::from_str("mx.example.org.")?)),
            ),
            (
                Name::from_str(ns_domain.as_str())?,
                RData::A(A(Ipv4Addr::new(10, 52, 1, 1))),
            ),
            (
                Name::from_str(subdomain.as_str())?,
                RData::AAAA(AAAA(Ipv6Addr::from_str(subdomain_ip6.as_str())?)),
            ),
        ]);
